...
export const fetchPendingRestaurant = createAsyncThunk("fetchPendingRestaurant", async () => {
  try {
...
export const updateRestaurnt = createAsyncThunk(
  "updateRestaurant",
  async (restaurant) => {
    try {
      const response = await axios.put(
        `${FETCH_URL}/${restaurant.id}`,
        restaurant,
        {
          headers: {
            Authorization: token,
...
export const createRestaurant = createAsyncThunk(
  ...
  async (restaurnt) => {
    try {
      console.log(token);
      const response = await axios.post(FETCH_URL, restaurnt, {
        headers: {
          Authorization: token,
          "Content-Type": "application/json",
        },
...
const restaurantSlice = createSlice({
  ...
  extraReducers(builder) {
    builder
      .addCase(fetchAllRestaurant.pending, (state) => {
      ...
      .addCase(updateRestaurnt.fulfilled, (state, action) => {
        ...
        const filteredRestaurant = state.restaurants.filter(
          (restaurant) => restaurant.id !== updatedRestaurnt.id
        );
        state.restaurants = [updatedRestaurnt, ...filteredRestaurant];
      })
      .addCase(deleteRestaurant.fulfilled, (state, action) => {
        const deletedRestaurant = action.payload;
        const filterRes = state.restaurants.filter(
          (res) => res.id !== deletedRestaurant.id
        );
...
export let getRestaurntById = (state, id) =>
  state.restaurants.restaurants.find((res) => res.id === id);
